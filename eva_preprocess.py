...
data_cfg = edict({
        ...
        'pre_train_file_mindrecord': './premindrecord/pre_train.mindrecord',
        'pre_test_file_mindrecord': './premindrecord/pre_test.mindrecord',
        'pre_val_file_mindrecord': './pre_valmindrecord/pre_val.mindrecord',
        'train_file_mindrecord': './mindrecord/train.mindrecord',
        'test_file_mindrecord': './mindrecord/test.mindrecord',
        'val_file_mindrecord': './val_mindrecord/val.mindrecord',
        'num_splits':4,
...
def data_preprocess(file, answer_vocab, answer_class, class_answer):
    ...
    for answer in answer_list:
        ...
        question_type_list.append(answer['question_type'])
        answer_type_list.append(answer['answer_type'])
        multi_answer_list.append(answer['multiple_choice_answer'])
    ...
    print("\n")
    return prepare_question, prepare_image, prepare_answer_seq, prepare_answer_class, question_type_list, answer_type_list, multi_answer_list


answer_vocab = dict()
answer_class = dict()
class_answer = dict()
test_question, test_image, test_answer_seq, test_answer_class, _, _, _ = data_preprocess('test', answer_vocab, answer_class, class_answer)
train_question, train_image, train_answer_seq, train_answer_class, _, _, _ = data_preprocess('train', answer_vocab, answer_class, class_answer)
val_question, val_image, val_answer_seq, val_answer_class, val_question_type, val_answer_type, val_multi_answer = data_preprocess('val', answer_vocab, answer_class, class_answer)

data_cfg.class_num = len(answer_class)
print("class_num:", len(answer_class))
print(val_question_type[0], val_answer_type[0], val_multi_answer[0])


def pre_data_prepare(cfg, dataset, save_file, question_list, image_list, answer_list, answer_class, question_type, answer_type, multi_answer) :
    ...
    data_schema = {
                    ...
                    "multiple_choice_answer" : {"type": "string"},
    ...
    for i in range(len(question_list)):
        ...
        data = {
            ...
            "multiple_choice_answer": multi_answer[i],
            "source_ids"     : np.asarray(features['source_ids'], dtype=np.int32),
            "source_mask"    : np.asarray(features['source_mask'], dtype=np.int32),
            "token_type_ids" : np.asarray(features['token_type_ids'], dtype=np.int32),
...
pre_data_prepare(data_cfg, "val", data_cfg.pre_val_file_mindrecord, val_question, val_image, val_answer_seq, val_answer_class, val_question_type, val_answer_type, val_multi_answer)
...
def load_pre_dataset(batch_size=1, data_file=None):
    ...
    ds = de.MindDataset(data_file,
                        columns_list=["question_type", "answer_type", "multiple_choice_answer",
                                      "source_ids", "source_mask", "token_type_ids",
                                      "target_ids", "image_vec", "label"],
                        shuffle=False)
    ...
    ds = ds.map(input_columns="target_ids", operations=type_cast_op)
...
def data_prepare(cfg, dataset, save_file) :
    ...
    for data in dataset.create_dict_iterator():
        question_type = data['question_type'].asnumpy()
        answer_type = data['answer_type'].asnumpy()
        multiple_choive_answer = data['multiple_choice_answer'].asnumpy()
        ...
        sequence_output, pooled_output, embedding_tables = bert_net(source_ids, token_type_ids, source_mask)
        #sequence_output = Tensor(np.zeros((4, 512, 1024)), dtype=mstype.float32)
        sequence_output = sequence_output.asnumpy()
        features = []
        for i in range(sequence_output.shape[0]):
            feature = {
                'answer_type': str(answer_type[i]),
                'question_type': str(question_type[i]),
                'multiple_choice_answer': str(multiple_choive_answer[i]),
...
#print(next(pre_val_data.create_dict_iterator()))
