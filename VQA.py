...
vqa_cfg = edict({
    #--------------------------------------nework config-------------------------------------
    'class_num': 8193, # total data has 8193 class,
    'fc_dim': 2048,
    ...
    'save_checkpoint_steps': 716,   #Save checkpoint steps, default is 80.
    'save_checkpoint_num':3,     #Save checkpoint numbers, default is 2.
...
vqa_net = VQAModel(resnet)
output_class = vqa_net(source_input, image_vec)

from vqa.vqa_for_train import VQATrainOneStepCell, VQANetworkWithLoss
...
def train(cfg, resnet):
    ...
    netwithloss = VQANetworkWithLoss(resnet, True, cfg.fc_dim, cfg.class_num)
    ...
    netwithgrads = VQATrainOneStepCell(netwithloss, optimizer=optimizer)
